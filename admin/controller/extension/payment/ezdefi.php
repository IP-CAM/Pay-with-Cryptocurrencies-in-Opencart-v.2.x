...
class ControllerExtensionPaymentEzdefi extends Controller {
    ...
    public function validateCoinConfig($coin_data) {
        ...
        if (isset($coin_data['coin_decimal']) && trim($coin_data['coin_decimal']) !== '' && (filter_var($coin_data['coin_decimal'], FILTER_VALIDATE_INT) === false || $coin_data['coin_decimal'] < 2 || $coin_data['coin_decimal'] > 30 )) {
            $this->error['decimal'] = $this->language->get('error_decimal');
        }
        if (isset($coin_data['max_currency_decimal']) && trim($coin_data['max_currency_decimal']) !== '' && (filter_var($coin_data['max_currency_decimal'], FILTER_VALIDATE_INT) === false || $coin_data['max_currency_decimal'] < 2 || $coin_data['max_currency_decimal'] > 30 )) {
